...
plot.browser=function(reads,genes,start,end,bin=1,col=NULL,gene.col=NULL,y=NULL,height=NULL,ymax=NULL,vline=NULL,line.gene=1)
{
	...
	if(length(genes)>0) for(i in 1:length(genes))
    {
		...
		if(gene.Disp.line == 0) continue	# No empty space -> skip displaying the gene
...
browser.draw=function(ymax=NULL,nbin=400,bin=NULL,read=T,vline=NULL)
{
    ...
    for(i in 1:(ncol-1)) y[i]=y[i]+sum(browser.param$heights[(i+1):ncol])
    if(is.null(browser.param$color)) col=rep('#000000',ncol)
    else col=browser.param$color
    max=plot.browser(browser.param$reads,browser.param$dispgene,browser.param$start,browser.param$end,bin=browser.param$bin,col=col,gene.col=browser.param$genecolor,y=y,ymax=ymax,height=browser.param$heights,vline=vline,line.gene=browser.param$gene.line)
