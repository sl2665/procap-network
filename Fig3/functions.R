...
plot.scatter.dist = function(x, y, cdc=NULL, file="test.pdf", xlim=NULL, ylim=c(-1,1), xlab="Distance (kb)", p.val=c(0.05,0.5,0.95), boxplot=F, cutoff=NULL) {
	if(!boxplot) {
		custom.scatter.open(file=file,xlab=xlab,ylab="Correlation coefficients",height=3,width=4)
		if(is.null(cutoff)) {
			custom.scatter.plot(x/1000,y,xlim=xlim/1000,ylim=ylim,height=3,width=4,col="jet",pval=F)
			...
			custom.scatter.plot(x/1000,y,xlim=xlim/1000,ylim=ylim,height=3,width=4,col="grayscale",pval=F)
			custom.scatter.plot(x[y>=cutoff]/1000,y[y>=cutoff],col="jet",add=T)
		}
